...
class ItemBasedCF(object):
    ...
    def calc_movie_sim(self):  #相似度区别
        ...
        # count co-rated users between items计算物品之间的共评级用户数-物品评分
    ...
    def recommend(self, user):
        ...
        for movie, rating in watched_movies.items():
            for related_movie, similarity_factor in sorted(self.movie_sim_mat[movie].items(),
                ...
                rank[related_movie] += similarity_factor * rating #相似特征*分数
        # return the N best movies
        return sorted(rank.items(), key=itemgetter(1), reverse=True)[:N]#返回TOP-N节目
